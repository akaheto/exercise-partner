...
export async function generateTechnicalSpec() {
  const doc = buildDocument([
    ...
    bullet("drizzle.config.ts loads env with plain dotenv/config, which reads only .env (local Postgres) — the same class of bug already fixed for runtime scripts in 1396bd1 via scripts/load-env.ts (.env.local first, matching Next.js precedence), but never applied to the config file drizzle-kit itself reads. Found 29 July 2026 when npm run db:generate hung on an interactive prompt while diffing against the wrong database. Migration 0007 was written by hand instead, matching the precedent already set by 0005/0006, and applied directly against Neon via npm run db:migrate. drizzle.config.ts itself remains unfixed and will hit this again the next time db:generate is needed."),
