...
export async function generateTechnicalSpec() {
  const doc = buildDocument([
    ...
    table(
      ["Table", "Key columns", "Notes"],
      [
        ["profiles", "id, display_name, avatar, preferred_weight_unit, experience_level, training_goal, pin_hash, pin_salt, pin_failed_attempts, pin_locked_until, onboarding_completed_at, created_at", "No longer credential-free: experience_level and training_goal were added in Epic M1 (they select the guidance pattern an exercise page shows), and pin_hash in M3 (PBKDF2-SHA256, 100k iterations, gates profile deletion). The schema comment in src/db/schema/app.ts still reads \"Lightweight — no credentials\" and is stale. onboarding_completed_at (migration 0007) is nullable and distinct from experience_level/training_goal on purpose — those default to Beginner/General, so their mere presence can't distinguish a real choice from a profile that never finished onboarding, which was the root cause of section 6's onboarding-redirect bug. pin_salt/pin_failed_attempts/pin_locked_until (migration 0008) close the PIN-salting and attempt-limiting gaps in section 6 — pin_salt is nullable and non-random on rows created before this migration, backfilled to the old fixed salt value so pre-existing PINs keep verifying rather than locking their owners out"],
    ...
    rich(
      "**Fixed — profile PINs are salted per-profile, with attempt limiting.** Every profile used to share one hardcoded salt, so identical PINs produced identical hashes and one precomputed table covered every profile's 4-6 digit keyspace at once. generatePinSalt() now produces a genuinely random salt per profile (migration 0008, profiles.pin_salt); existing profiles' pin_hash — computed under the old fixed salt — keeps verifying via an explicit backfill of that same salt value onto their row, rather than a forced reset with no reset UI to support it. A regression test pins hashPin's output for a known input under the legacy salt so this compatibility can't silently break later. Verified directly against the database: two throwaway profiles created with the identical PIN produce different pin_hash values. Attempt limiting: 5 consecutive wrong guesses lock deletion out for 15 minutes (profiles.pin_failed_attempts/pin_locked_until, checked before a guess is even hashed), verified live — a 6th attempt with the *correct* PIN is still refused while locked. verifyPin compares with crypto.timingSafeEqual rather than ===.",
    ),
