  ...
  const doc = buildDocument([
    ...
    table(
      ...
      [
        ...
        ["K2", "Critical-path tests for each feature area", S.inProgress, "Playwright covers Workout Mode (H6), PIN salting/lockout, and admin-cookie forgery rejection. All 9 specs run and pass again as of the 2 September 2026 K4 pass, after 3 had silently broken (see K4). Still no e2e coverage of the onboarding completion flow itself or admin dashboard functionality beyond auth"],
        ...
        ["K4", "Manual QA against this plan before calling v1 done", S.inProgress, "First real pass run 2 September 2026: the full Playwright suite (9 specs) was executed for the first time in a while and 3 of 9 were found silently broken by real, since-shipped product changes that nobody had re-verified e2e against — not flaky tests, but stale ones. (1) Epic P4 (this session) made /profile the admin-only all-profiles view; 3 specs still navigated there expecting the universal \"Add a profile\" form. (2) Epic N5's WeightToggle/RepToggle redesign (tap-to-cycle buttons) replaced an earlier NumberStepper (fillable input + separate +/- buttons) that workout-mode.spec.ts still assumed. All 3 fixed and passing again; the header profile-switcher's Name/PIN fields also gained real <Label> elements (previously placeholder-only, an accessibility gap fixed as a side effect of making them reliably testable). Not yet done: a pass over the non-Playwright-covered surfaces (onboarding completion, admin dashboard functionality)"],
        ["K5", "Deploy and verify in production", S.done, "This row was stale — found during a 2 September 2026 pass. Vercel, git-connected (push to main auto-deploys); the site has been live and repeatedly verified against the real production deployment (not localhost) across multiple sessions, most recently for the Workout Library search-box and documentation-sync work above"],
      ],
      EPIC_WIDTHS,
    ),
    ...
    table(
      EPIC_COLS,
      [
        ["Q1", "Source schema, scraper and import for packaged programs", S.done, "Three new source tables (source_workout_programs, source_workout_program_days, source_workout_program_exercises; migrations 0009-0010), following the same imported-vs-app-owned separation as the rest of the source layer. Grew in three stages: 4 programs (initial evaluation) -> 16 (29 July 2026, surfacing h2-level day headings, a description paragraph between heading and table, a literal \"#\" in \"Workout #1\", and Sets-column-less EMOM/circuit tables) -> 613 (2 September 2026, via a new scripts/import-workout-extract.ts reading a fresh site-wide extract instead of one-URL-at-a-time scraping). The 613-program import matches existing rows by canonical URL so hand-curated program_id values survive re-import, batches each program's day/exercise inserts (2 bulk statements regardless of program size, not one row-at-a-time await) after an initial one-row-per-await version measured at ~6 hours projected for the full batch, and fixed a URL-construction bug that double-prepended the domain. Corrected 2 September 2026 (same day, user-reported): 313 of 613 programs showed a \"days/week\" badge that disagreed with their actual day-by-day content (e.g. a \"3 Day\" program with 4 real days), and 47 of those also carried mostly-empty duplicate days. Root cause confirmed against the source extract's own \"Source Consistency Notes\" column (317 programs flagged) rather than guessed: for narrative-format source pages, the extract fabricates placeholder day-title scaffolding from the stated Days Per Week count, alongside the one real day that actually holds the parsed content — both land in the Workout Days sheet with zero way to distinguish them there except that the scaffolding rows never appear as any exercise's linked day. Fixed by dropping any day with zero linked exercises before storage, and deriving the daysPerWeek badge from the real (filtered) day count rather than trusting the source's own field. Re-run: 1,778 -> 1,686 days, zero exercise rows lost (verified), zero mismatches/duplicates/empty days remaining (verified by direct query). A related, separately-caused bug found during verification and fixed alongside: the program detail page's daysPerWeek/durationWeeks badges used a bare `&&` truthiness check, rendering a stray literal \"0\" for any program with a real 0 value — the library list page had already been fixed for this earlier the same day, but the detail page never got the matching fix. Final state: 613 programs, 1,686 days, 15,839 exercise rows, 13,237/15,839 (83.6%) matched to a library exercise by URL"],
    ...
    table(
      ...
      [
        ...
        ["55", "RESOLVED — reviewed 2 September 2026, as requested when the July 30 login-outage bug fix left open whether the auth system itself is overengineered. Finding: it is not — the \"three credentials\" framing overstated the real surface. SITE_PASSWORD and ADMIN_TOKEN are genuine, independent access-control credentials; the per-profile PIN gates only that profile's deletion, never general access; and activeProfileId is correctly unsigned, because it is not a security boundary at all — profiles are not per-person logins (item 1), so anyone already past the site password is trusted to view any profile by design. The \"redundant\" per-page checks are not redundancy for its own sake either: requireSiteSession() runs in every Server Action in addition to src/proxy.ts's route gate because Server Actions are reachable directly by request, independent of middleware route matching — documented in Next.js's own Proxy docs and in src/lib/require-site-session.ts's comment. Removing it would reopen exactly the July 30 class of bug, not simplify anything real. One genuine, unrelated bug was found while reading this code and fixed alongside: the header's \"Switch profile → Add a profile\" mini-form (profile-switcher-dialog.tsx, written in Epic C) never had a PIN field, so it could never succeed once Epic M3 made createProfile() require one three days later — every other creation entry point (onboarding, home, /profile) already sent one. Fixed by adding the missing field; a mutation-tested regression test (profile-switcher-dialog.test.tsx) confirms it fails against the pre-fix code and passes against the fix.", "Resolved"],
        ...
        ["57", "RESOLVED — public/enhancements.json (the /admin/enhancements dashboard's data) and ENHANCEMENTS.docx (the canonical source per CLAUDE.md) had drifted for over a month with nothing to catch it. Fixed 2 September 2026: scripts/docs/enhancements.ts now exports its three data arrays (IMPLEMENTED, NOT_YET_IMPLEMENTED, REJECTED_DEFERRED) as the single source both the .docx builder and a new generateEnhancementsJson() consume; npm run docs writes public/enhancements.json from the same call. The admin page's shape changed to match what the source doc actually contains (Implemented/Not Yet Implemented/Rejected-Deferred, with whatItDoes and effort fields) rather than the old ad-hoc priority/status split, which had never corresponded to real fields in enhancements.ts. Verified rendering end-to-end in a real browser via a temporary auth bypass (reverted immediately after, no credentials involved).", "Resolved"],
        ["58", "DECIDED — /build/library's listWorkoutProgramsByCategory has no pagination or row cap, now querying across 613 programs (up from the 4-36 the page was built and tested against). Measured at 241ms for the full unfiltered query in production — not currently a problem. Decided 2 September 2026: leave unpaginated rather than build speculative complexity; a code comment on the query (src/db/queries/workout-programs.ts) records the measurement and names ~1s or a few thousand programs as the threshold to revisit at.", "Decided"],
      ],
      [6, 79, 15],
    ),
    ...
    table(
      ...
      [
        [
          formatDate(),
          "User-reported bug fixed: some imported Workout Library programs showed \"N days/week\" badges that disagreed with their actual day-by-day content (e.g. a \"3 Day\" program with 4 real training days), and some showed several mostly-empty duplicate days. Traced to the source extract's own documented behavior for narrative-format pages (317 of 613 programs flagged in its own \"Source Consistency Notes\" column, confirmed by reading the raw sheets directly): it fabricates placeholder day-title scaffolding from the stated Days Per Week count alongside the one real day that actually holds the parsed content, both landing in the same sheet with no way to tell them apart except that the scaffolding rows link to zero exercises. scripts/import-workout-extract.ts now drops any day with zero linked exercises before storing it, and derives the daysPerWeek badge from the real filtered day count instead of trusting the source's own field. Re-ran against production: 1,778 -> 1,686 days, zero exercise rows lost, zero mismatches or duplicates remaining (verified by direct query, not assumed). Also fixed a related but separately-caused bug found during verification: the program detail page's badges used a bare `&&` truthiness check and rendered a stray literal \"0\" for a real 0-value program — the list page had already been fixed for this exact bug class earlier the same day; the detail page never got the matching fix.",
        ],
        [
          formatDate(),
          "K4's first real manual-QA pass: ran the full Playwright suite (9 specs) end to end for the first time in a while and found 3 silently broken by real, already-shipped product changes nobody had re-verified against — not flakiness, staleness. Epic P4 (this session) made /profile admin-only, breaking 3 specs' assumption that its \"Add a profile\" form was universally reachable; fixed by routing them through the header's profile switcher instead, which stays reachable regardless of admin status. Epic N5's tap-to-cycle WeightToggle/RepToggle redesign had silently obsoleted workout-mode.spec.ts's fillable-input assumptions; rewritten to match. All 9 specs pass again. Confirmed no throwaway test data was left in production afterward.",
        ],
        [
          formatDate(),
          "Item 55 (auth architecture review, open since the July 30 login-outage fix) resolved: the system is not overengineered — SITE_PASSWORD and ADMIN_TOKEN are the only real access-control credentials, the profile PIN gates only deletion, activeProfileId is correctly unsigned since profiles were never meant to be a security boundary, and the per-Server-Action requireSiteSession() calls are documented defense-in-depth (Server Actions bypass middleware route matching), not redundancy. Found and fixed one real, unrelated bug while reading the code: the header's \"Add a profile\" mini-form had no PIN field, so it silently failed every submission since Epic M3 added the PIN requirement on 28 July — every other profile-creation entry point already sent one. Fixed with a mutation-tested regression test that fails against the pre-fix code.",
        ],
        [
          formatDate(),
          "Worked through PROJECT_PLAN.docx section 4's remaining open items. Item 57 (enhancements.json/ENHANCEMENTS.docx drift) fixed properly this time: scripts/docs/enhancements.ts now exports its data as the single source both the .docx and a new public/enhancements.json generator read, and the admin dashboard's shape was corrected to match what the source doc actually contains rather than a set of fields (priority/status) that had never existed in it. Item 58 (unbounded /build/library query) decided rather than built — left unpaginated at 613 rows/241ms, with a code comment recording the revisit threshold. Item K5's stale \"Not Started\" deploy status corrected to Done — the site has in fact been live and repeatedly verified in production for some time.",
        ],
        [
          formatDate(),
