...
export async function generateProjectPlan() {
  const doc = buildDocument([
    ...
    table(
      EPIC_COLS,
      [
        ["Q1", "Source schema, scraper and import for packaged programs", S.done, "Three new source tables (source_workout_programs, source_workout_program_days, source_workout_program_exercises; migrations 0009-0010), following the same imported-vs-app-owned separation as the rest of the source layer. scripts/import-workout-programs.ts fetches, caches locally and parses each program page, handling three distinct page-structure variants found on the real site (h4 \"Day N\", h4 + weekday name, h3 \"Workout N\"). 4 programs imported for evaluation: 4 Day Maximum Mass Workout, Upper/Lower 4 Day Bodybuilding Workout, 3 Day PPL for Beginners, 12 Week Fat Destroyer — 106 exercise rows across 15 training days, 104/106 matched to an existing library exercise by URL (the 2 unmatched genuinely have no linked exercise on the source page)"],
        ...
        ["Q3", "\"Add to my saved workouts\" action", S.notStarted, "Deliberately deferred: creates one workout (with blocks/items) per training day when requested. Waiting on user evaluation of Q1/Q2's real imported data before building the write path"],
    ...
    table(
      ...
      [
        ...
        ["54", "Source day tables sometimes put a duration in the Sets column and \"Burn\" in the Reps column instead of a number (e.g. \"5 Minutes\" / \"Burn\") for timed burnout sets at the end of a muscle group's exercises — confirmed against the raw source HTML, not a parser bug. Imported and displayed verbatim under the existing Sets/Reps/Rest headers rather than re-interpreted, consistent with section 4's rule against presenting derived data as sourced fact — but it reads oddly in a generic Sets/Reps table and may be worth a dedicated \"burn set\" treatment if/when Epic Q continues past the evaluation checkpoint.", "Open"],
      ],
      [6, 79, 15],
    ),
