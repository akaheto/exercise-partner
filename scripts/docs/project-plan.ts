  ...
  const doc = buildDocument([
    ...
    table(
      ...
      [
        ...
        ["49", "drizzle.config.ts loads env with plain dotenv/config, which reads only .env (local Postgres) — the same mismatch already fixed for runtime scripts in 1396bd1 via scripts/load-env.ts (.env.local first, matching Next.js precedence), but never applied to drizzle-kit itself. Found 29 July 2026 when npm run db:generate hung on an interactive rename-resolution prompt while diffing against local Postgres instead of the Neon database the app and scripts/migrate.ts actually use. Migration 0007 was written by hand instead (matching the precedent already set by 0005/0006), applied directly with npm run db:migrate against Neon, and confirmed present via a live column query — the same workaround, not a fix. drizzle.config.ts itself is unchanged and will hit this again next time db:generate is needed.", "Open"],
