...
export async function generateProjectPlan() {
  const doc = buildDocument([
    ...
    p(
      "Started 29 July 2026, by request. The eventual goal: a single \"My Profile\" destination for everyone, and \"Profile\" (the all-profiles switcher and creation view) restricted to the site admin. There is no admin concept at the profile level today — see item 51 — so this epic begins with the parts that don't depend on that decision.",
      { muted: true },
    ),
    table(
      EPIC_COLS,
      [
        ...
        ["P4", "Gate /profile to the admin", S.notStarted, "Not started, and not decided how: there is no profiles.is_admin column or any other per-profile admin concept today — admin access is currently a completely separate system (SITE_PASSWORD + ADMIN_TOKEN, an admin_session cookie, src/lib/admin-auth.ts) with no link to which profile happens to be active. See item 51 for the open question this blocks on"],
      ],
      EPIC_WIDTHS,
    ),
    ...
    table(
      EPIC_COLS,
      [
        ["Q1", "Source schema, scraper and import for packaged programs", S.done, "Three new source tables (source_workout_programs, source_workout_program_days, source_workout_program_exercises; migrations 0009-0010), following the same imported-vs-app-owned separation as the rest of the source layer. scripts/import-workout-programs.ts fetches, caches locally and parses each program page. Expanded 29 July 2026 from the initial 4-program evaluation batch to 16 programs (64 training days, 408 exercise rows, 370/408 matched to an existing library exercise by URL) spanning splits, beginner programs, women's programs, bodyweight/home options and single-lift specialization programs, surfacing several more real page-structure variants beyond the original three: day headings at the h2 level (not just h3/h4); a table separated from its heading by an intervening description paragraph rather than sitting immediately after it; \"Workout #1\" (a literal # before the number, which the original Day/Workout regex didn't tolerate); and tables with no Sets column at all (EMOM/circuit-style pages have only Exercise/Reps/Rest) — sets/reps are now read by matching the header row's own labels rather than a fixed column position, so a genuinely absent column reads as null instead of silently reading another column's data under the wrong name. A real false-positive was also found and fixed during this expansion: the broadened heading search could land on an unrelated table under a generic section heading (e.g. a weight-progression chart under \"The Workouts\", not a day's exercises) — a candidate table is now only accepted if its header row actually contains an \"Exercise\" column. The 2 unmatched exercises from the original 4-program batch, and the further unmatched rows in this larger batch, genuinely have no linked exercise on the source page — verified against the raw HTML, not a parser gap"],
        ["Q2", "Read-only browse UI", S.done, "/build/library (list, badge chips for goal/level/days-per-week/duration/audience) and /build/library/[id] (day-by-day DataTable of exercise/sets/reps/rest, matched exercises link to their library page). Added as a third, outline-styled card on /build (\"Choose from the library\") — outline deliberately distinguishes it as not yet feature-complete like the other two options. A real bug was found and fixed during visual verification: the day heading combined the DB dayNumber with a focus string that already contained its own \"Day N -\" prefix scraped from the source heading, rendering as \"Day 1 — Day 1 - Back & Biceps\"; the scraper now strips that leading label before storing focus"],
        ...
        ["Q4", "Facet-filtering UI (goal/duration/equipment/audience)", S.notStarted, "Deferred until there is enough imported content for filtering to mean anything — 4 programs does not"],
      ],
      EPIC_WIDTHS,
    ),
    ...
    table(
      ...
      [
        ...
        ["51", "OPEN — Epic P4 (gate /profile to the admin) is blocked on a decision this project has never needed before: what does \"administrator\" mean at the level of a profile? Today admin access (SITE_PASSWORD + ADMIN_TOKEN, an admin_session cookie) is entirely separate from the profile system — a person can have an active admin session and any profile, or no profile, active at the same time; there is no profiles.is_admin column, and profiles carry no credential beyond an optional PIN that gates only their own deletion. Candidate approaches: (a) gate /profile on getAdminSessionStatus() directly, i.e. \"admin\" means \"currently signed into /admin in this browser\" — cheapest, reuses existing auth, but ties a navigation-visibility decision to a session that expires every 4 hours; (b) add a real profiles.is_admin flag, a designated admin profile distinct from the session-based /admin gate — more principled, but a second, overlapping notion of \"admin\" in the same app; (c) don't gate by identity at all, just stop linking to /profile from nav for everyone and leave it reachable by direct URL as a quasi-admin back door — cheapest of all, weakest. Not decided; P4 is Not Started until it is.", "Open"],
