...
export async function generateProjectPlan() {
  const doc = buildDocument([
    ...
    table(
      EPIC_COLS,
      [
        ["M1", "Onboarding flow", S.blocked, "Built but broken, discovered 29 July 2026. /onboarding renders four steps (name → experience level → training goal → completion), but only step 1 is reachable: creating the profile revalidates the layout, the server component sees an active profile and redirects to /exercises. Experience level and training goal are therefore never chosen and keep their defaults. See section 4, item 47 — the fix is a product decision, not yet made"],
    ...
    table(
      ...
      [
        ...
        ["47", "ONBOARDING STEPS 2-4 ARE UNREACHABLE, and have been since Epic M1 shipped. createProfile() calls revalidatePath(\"/\", \"layout\"), which re-runs the /onboarding server component; that component redirects to /exercises whenever an active profile exists, and step 1 has just created one. So a new user completes step 1 and is dropped straight into the exercise library, never choosing an experience level or training goal. Both keep their defaults (Beginner / General). Combined with item 40 — the seeder assigning every exercise a beginner_* pattern — this means the level-and-goal personalisation Epic L was built for has effectively never varied for anyone. Confirmed by driving the flow in a browser and logging the URL after step 1: it is /exercises. Verified as the cause by disabling the redirect, after which all four steps run correctly; the guard was then restored, so the bug is still present. NOT FIXED — the fix is a product decision. Removing the guard lets anyone revisit /onboarding, which may be the desired behaviour; the alternative is an explicit onboarding-completed flag on the profile, since experience_level and training_goal cannot distinguish \"chose Beginner\" from \"never asked\".", "Open"],
