...
export async function generateProjectPlan() {
  const doc = buildDocument([
    ...
    p(
      "Sequenced deliberately after Epic N: a supplied set of 1,218 anatomical renders sits alongside the custom SVG muscle diagram built in Epic D. Assets delivered 29 July 2026 to Images/Exercise_Muscle_Group_Diagrams_1218/ in the Drive folder.",
      { muted: true },
    ),
    callout(
      "Scope decided, not as originally sketched",
      "The plan below originally read \"replace the SVG diagram\" (O3). That changed once the palette conflict (section 4, item 43) was actually decided: the supplied renders' orange/navy legend is baked into the raster and contradicts the style guide's teal involvement ramp, and the choice made was to keep the teal ramp as the app's own visual language rather than amend it. That only makes sense if the hand-built SVG diagram — the thing the teal ramp exists to colour — stays in place. So the render is additive: a second, clearly-framed \"Anatomical reference\" plate beneath the existing diagram, not a replacement for it.",
    ),
    table(
      EPIC_COLS,
      [
        ...
        ["O3", "Add the anatomical reference plate to the exercise detail page", S.done, "New MuscleDiagramPhoto component, added beside (not replacing) the existing MuscleDiagram. Fails silent — renders nothing — if NEXT_PUBLIC_MUSCLE_DIAGRAM_BASE_URL is unset or the specific image 404s, since the hand-built diagram above it already covers that exercise; a broken-image icon was judged worse than a quietly absent second panel. Not added to Workout Mode's session runner (src/components/session/session-runner.tsx) — a full photographic plate competes for space that screen needs for logging a set; the existing compact SVG stays there unchanged. 6 unit tests, mutation-checked"],
        ["O4", "Accessibility and dark theme", S.done, "Alt text built from experience_level/primary_muscle/secondary_muscles in the database, not from the image (item 42) — the baked-in legend is invisible to a screen reader and can't reflect an override correction. Dark theme: the plate keeps a light background and border in both themes deliberately (item 44's chosen treatment, \"frame as a plate\") rather than trying to invert a shaded photographic render, verified in a real browser in both themes. Palette conflict (item 43) resolved by NOT touching VISUAL_STYLE_GUIDE.docx's teal ramp — see the callout above"],
      ],
      EPIC_WIDTHS,
    ),
    callout(
    ...
    table(
      ...
      [
        ...
        ["43", "DECIDED — the diagrams use orange/navy for primary/secondary involvement, baked in and unchangeable; VISUAL_STYLE_GUIDE.docx specifies a single-hue teal ramp for the same purpose. Resolved by NOT amending the style guide: the teal ramp stays the app's own visual language for the hand-built MuscleDiagram (Epic D), which is kept rather than replaced specifically so the ramp still has something to colour. The supplied render sits in its own bordered \"Anatomical reference\" plate with its own caption naming the mismatch explicitly, so it reads as an inserted photograph with its own legend, not as the app's design language contradicting itself.", "Decided"],
